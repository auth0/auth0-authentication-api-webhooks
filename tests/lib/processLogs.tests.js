...
describe('processLogs', () => {
  ...
  it('should send logs to fake webhook', (done) => {
    ...
    nock('https://test.auth0.com')
      ...
      .reply(function() {
        const logs = [
          ...
          { _id: 1, date: new Date(), type: 'ssa', details: { request: { path: '/api/v2/users' } } },
          { _id: 2, date: new Date(), type: 's', details: { request: { path: '/api/v2/test' } } }
        ];
    ...
    const res = {
      json: (data) => {
        expect(receivedLogs.length).to.equal(2);
        expect(data.status.logsProcessed).to.equal(2);
        expect(data.status.checkpoint).to.equal(2);
        done();
      }
    };
